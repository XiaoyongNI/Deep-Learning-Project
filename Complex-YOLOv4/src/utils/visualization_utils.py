...
def invert_target(targets, calib, img_shape_2d, cfg, patch_id,RGB_Map=None):
    predictions = targets
    predictions = kitti_bev_utils.inverse_yolo_target(predictions, cfg.boundary[patch_id])
    ...
    if RGB_Map is not None:
        ...
        target = kitti_bev_utils.build_yolo_target(labels,cfg.boundary[patch_id])
        kitti_bev_utils.draw_box_in_bev(RGB_Map, target , cfg)

    return objects_new


def predictions_to_kitti_format(img_detections, calib, img_shape_2d,cfg,patch_id,img_size, RGB_Map=None):
    predictions = []
    ...
    predictions = kitti_bev_utils.inverse_yolo_target(np.array(predictions), cfg.boundary[patch_id])
    if predictions.shape[0]:
        predictions[:, 1:] = transformation.lidar_to_camera_box(predictions[:, 1:], calib.V2C, calib.R0, calib.P)
