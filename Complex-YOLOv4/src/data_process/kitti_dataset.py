...
class KittiDataset(Dataset):
    def __init__(self, dataset_dir, config=None, mode='train',lidar_transforms=None, aug_transforms=None, multiscale=False,
        ...
        self.cfg = config
        self.img_size = self.cfg.BEV_WIDTH
    ...
    def load_img_only(self, index):
        ...
        sample_id,patch_id = self.sample_id_list[index]
        lidarData = self.get_lidar(sample_id)
        b = kitti_bev_utils.removePoints(lidarData, self.cfg.boundary[patch_id])
        rgb_map = kitti_bev_utils.makeBVFeature(b, self.img_size,
                                                self.cfg.DISCRETIZATION[patch_id], self.cfg.boundary[patch_id])
        img_file = os.path.join(self.image_dir, '{:06d}.png'.format(sample_id))
        return (img_file,patch_id), rgb_map
    ...
    def load_img_with_targets(self, index):
        ...
        if self.lidar_transforms is not None:
            # Note that when using lidar_transforms, the bounding box can't be accurately projected back to the front view image
            lidarData, labels[:, 1:] = self.lidar_transforms(lidarData, labels[:, 1:])

        b = kitti_bev_utils.removePoints(lidarData, self.cfg.boundary[patch_id])
        rgb_map = kitti_bev_utils.makeBVFeature(b, self.img_size,self.cfg.DISCRETIZATION[patch_id], self.cfg.boundary[patch_id])
        target = kitti_bev_utils.build_yolo_target(labels, self.cfg.boundary[patch_id])
        img_file = os.path.join(self.image_dir, '{:06d}.png'.format(sample_id))
        ...
        return (img_file,patch_id), rgb_map, targets
    ...
    def load_mosaic(self, index):
        ...
        for i, index in enumerate(indices):
            ...
            c, h, w = img.size()  # (3, 64, 64), torch tensor
    ...
    def remove_invalid_idx(self, image_idx_list):
        ...
        for sample_id in image_idx_list:
            ...
            if not noObjectLabels:
                labels[:, 1:] = transformation.camera_to_lidar_box(labels[:, 1:], calib.V2C, calib.R0,
                                                                   calib.P)  # convert rect cam to velo cord
            for patch_id in range(int(self.cfg.PATCH_NUM)**2):
                valid_list = []
                for i in range(labels.shape[0]):
                    if int(labels[i, 0]) in cnf.CLASS_NAME_TO_ID.values():
                        if self.check_point_cloud_range(labels[i, 1:4],self.cfg.boundary[patch_id]):
                            valid_list.append(labels[i, 0])

                if len(valid_list) > 0:
                    sample_id_list.append((sample_id,patch_id))

        return sample_id_list

    def check_point_cloud_range(self, xyz, bc):
        """
        :param xyz: [x, y, z]
        :return:
        """
    ...
    def collate_fn(self, batch):
        ...
        if self.img_size != img_size_origin:
