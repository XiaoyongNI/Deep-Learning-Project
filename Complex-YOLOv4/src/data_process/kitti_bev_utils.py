...
def makeBVFeature(PointCloud_, bevsize, Discretization, bc):
    Height = bevsize + 1
    Width = bevsize + 1
    # Discretize Feature Map
    PointCloud = np.copy(PointCloud_)
    PointCloud[:, 0] = np.int_(np.floor((PointCloud[:, 0] - bc['minX'])/ Discretization))
    PointCloud[:, 1] = np.int_(np.floor((PointCloud[:, 1] - bc['minY'])/Discretization))
    ...
    RGB_Map[2, :, :] = densityMap[:bevsize, :bevsize]  # r_map
    RGB_Map[1, :, :] = heightMap[:bevsize, :bevsize]  # g_map
    RGB_Map[0, :, :] = intensityMap[:bevsize, :bevsize]  # b_map
...
def build_yolo_target(labels, bc):
    target = []
...
def draw_box_in_bev(rgb_map, target, cfg):
    for j in range(50):
        ...
        x = target[j][1] * cfg.BEV_WIDTH
        y = target[j][2] * cfg.BEV_HEIGHT
