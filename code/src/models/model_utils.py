...
if __name__ == '__main__':
    ...
    configs.device = torch.device('cuda:1') if torch.cuda.is_available() else "cpu"
    model = create_model(configs).to(device=configs.device)
    sample_input = torch.randn((1, 3, 64, 64)).to(device=configs.device)
